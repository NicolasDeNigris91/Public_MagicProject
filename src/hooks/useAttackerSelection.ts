...
export function useAttackerSelection() {
  ...
  // blocker between preview and commit — guaranteed today by the
  // animator's serial queue and the isAnimating guards at every
  // attack entry point (player + AI).
  const runCombat = async (attackerId: CardId, blockerId: CardId | null) => {
  ...
  const handleBattlefieldActivate = (card: ICard) => {
    if (useCombatStore.getState().isAnimating) return;
  ...
  // per render would invalidate that memo and rebuild the action
  // array each time the inspector is open.
